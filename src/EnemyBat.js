...
var EnemyZonbi = cc.Sprite.extend({
  ctor: function() {
    this._super();
    ...
    var animationframe = [];
    //スプライトフレームを格納する配列
    var texture = cc.textureCache.addImage(res.zonbi_frames);
    for (i = 0; i < 2; i++) {
      for (j = 0; j < 4; j++) {
        //スプライトフレームを作成
  ...
  update: function(dt) {
    ...
    if (this.FrameCount % 4 == 0) {
      ...
      //offset_x = offset_x*Math.sin(offset_x);
      //蝙蝠のｘ移動速度をプレイヤとこうもりの間の距離の0.05倍にする
      var velocity_x = lerp(this.velocity.x, offset_x, 0.0005);
      var velocity_y = this.velocity.y;
